...
export function getDirectionToClosestParcel(agent: MyAgent): Direction | undefined {
  const you = agent.you;
  if (!you || agent.parcelsSensing.length === 0) return;

  // Trova solo i pacchi non ancora raccolti
  const parcelsOnGround = agent.parcelsSensing.filter(p => p.carriedBy !== you.id);
  if (parcelsOnGround.length === 0) return;
  ...
  if (Math.abs(dx) > Math.abs(dy)) {
    ...
    if (dy !== 0) directions.push(dy > 0 ? "up" : "down");
  } else {
    if (dy !== 0) directions.push(dy > 0 ? "up" : "down");
    if (dx !== 0) directions.push(dx > 0 ? "right" : "left");
  }
...
export function getDirectionToClosestDeliveryPoint(agent: MyAgent): Direction | undefined {
  const you = agent.you;
  if (!you) return;
