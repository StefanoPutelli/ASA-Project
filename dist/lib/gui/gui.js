...
export async function updateGui(agent) {
    // Update map box
    ...
    const agentsContent = agent.agentsSensing[agent.agentsSensing.length - 1] ? agent.agentsSensing[agent.agentsSensing.length - 1].map(otherAgent => {
        ...
        const arrow = Array.isArray(direction) && direction.length === 2 ? (direction[0] === 0 && direction[1] === -1 ? '↑ (North)' :
    ...
    const intentionsContent = agent.intentions.slice(-Math.floor(intentionsBox.height) + 2)
